...
class UnixByteTransport implements ByteTransport {
	...
	private readonly markLocalClose: () => void;
	...
	private write(chunk: Uint8Array): Promise<void> {
		...
		return new Promise<void>((resolve, reject) => {
			...
			const cleanup = (): void => {
				...
				if (closeListener) this.socket.off("close", closeListener);
			};
			...
			closeListener = () => fail(new Error("Unix transport closed during write"));
