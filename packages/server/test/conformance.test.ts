...
async function startServer(
	...
	overrides: Partial<PiServerOptions> = {},
): Promise<{ server: PiServer; backend: ConformanceBackend }> {
	const directory = await mkdtemp(join(tmpdir(), "pi-server-conformance-"));
...
async function connect(server: PiServer): Promise<WireClient> {
	const client = await connectUnix(server.unixSocketPath!);
	clients.add(client);
	return client;
}
...
describe("Unix transport conformance", () => {
	...
	test("gracefully closes connections, sessions, and listener resources", async () => {
		...
		backend.seed("first");
		const { server } = await startServer(backend);
		const socketPath = server.unixSocketPath;
		const client = await connect(server);
		await client.hello();
