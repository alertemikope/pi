...
export class UnixListener {
	...
	constructor(options: ListenerOptions) {
		...
		this.path = options.listener.path;
		this.mode = options.listener.mode ?? DEFAULT_SOCKET_MODE;
	}
	...
	private acceptSocket(socket: Socket): void {
		...
		const handler = this.options.accept(connection);
...
class UnixByteConnection implements ByteConnection {
